...
Examples:
  ...
  bash run_stock_a800_1gpu.sh "your_tushare_token" --no-setup --no-download-model
  bash run_stock_a800_1gpu.sh "your_tushare_token" --python-bin "$(command -v python3.10)"
  MODEL_ID=Qwen/Qwen3-4B MODEL_DIR=/kunlun_data/temp_ag_rl/model/Qwen3-4B bash run_stock_a800_1gpu.sh "your_tushare_token"
  bash run_stock_a800_1gpu.sh "your_tushare_token" -- --total-epochs 1 --max-tasks 800
EOF
}
...
export STOCK_AGENT_A800_MAX_PROMPT_LENGTH="${STOCK_AGENT_A800_MAX_PROMPT_LENGTH:-1536}"
export STOCK_AGENT_A800_MAX_RESPONSE_LENGTH="${STOCK_AGENT_A800_MAX_RESPONSE_LENGTH:-512}"
export STOCK_AGENT_A800_ROLLOUT_GPU_MEMORY_UTILIZATION="${STOCK_AGENT_A800_ROLLOUT_GPU_MEMORY_UTILIZATION:-0.30}"
export STOCK_AGENT_A800_ROLLOUT_MAX_MODEL_LEN="${STOCK_AGENT_A800_ROLLOUT_MAX_MODEL_LEN:-2048}"
export STOCK_AGENT_A800_ROLLOUT_MAX_NUM_BATCHED_TOKENS="${STOCK_AGENT_A800_ROLLOUT_MAX_NUM_BATCHED_TOKENS:-3072}"
...
export STOCK_AGENT_A800_SAMPLE_STRIDE="${STOCK_AGENT_A800_SAMPLE_STRIDE:-8}"
export STOCK_AGENT_A800_MAX_TASKS="${STOCK_AGENT_A800_MAX_TASKS:-1000}"
export TORCH_CUDA="${TORCH_CUDA:-cu124}"
export MODEL_ID="${MODEL_ID:-Qwen/Qwen3-0.6B}"
export MODEL_DIR="${MODEL_DIR:-$DEFAULT_PERSIST_ROOT/model/Qwen3-0.6B}"
export STOCK_AGENT_REQUIRE_PYTHON_MINOR="${STOCK_AGENT_REQUIRE_PYTHON_MINOR:-3.10}"
